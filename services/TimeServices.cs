...
namespace templumStudii.services {
    ...
    public class TimeService
    {
        ...
        public async Task<TimeResponse> DefineAsync(TimeSpan definedTime, int userId)
        {
            ...
            var time = new Time { DefinedTime = definedTime, AccumulatedTime = TimeSpan.Zero, StartedAt = null, UserId = userId };
            var saved = await _repository.AddAsync(time);
            return new TimeResponse { Id = saved.Id, DefinedTime = saved.DefinedTime, AccumulatedTime = saved.AccumulatedTime };
        }
